...
def load_dim_geography(geo_df: pd.DataFrame, engine) -> None:
    ...
    # Format the columns in a vectorized way first to ensure deduplication catches case mismatches
    geo_df = geo_df.copy()
    geo_df["zip_code_prefix"] = geo_df["geolocation_zip_code_prefix"].astype(str).str.zfill(5)
    geo_df["city"] = geo_df["geolocation_city"].fillna("").astype(str).str.strip().str.title()
    geo_df["state"] = geo_df["geolocation_state"].fillna("").astype(str).str.strip().str.upper()
    
    # Remove any empty city values
    geo_df = geo_df[geo_df["city"] != ""]
    
    # Now deduplicate by the formatted zip_code_prefix and city
    geo_dedup = geo_df.drop_duplicates(subset=["zip_code_prefix", "city"])
...
def load_fact_orders(
    ...
    # Aggregate order items by order_id to calculate totals and keep order_id unique
    agg_items = items_df.groupby("order_id").agg({
        "price": "sum",
        "freight_value": "sum",
        "product_id": "first",
        "seller_id": "first"
    ...
    merged = pd.merge(orders_df, agg_items, on="order_id", how="left")
...
def load_dim_reviews(reviews_df: pd.DataFrame, engine) -> None:
    ...
    # Deduplicate by both review_id and order_id to satisfy both uniqueness constraints
