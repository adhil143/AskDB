...
def get_schema_for_database(db_key: str = "olist_db") -> list[dict[str, Any]]:
    """Return the static semantic schema for olist_db, or dynamically inspect schema for custom databases."""
    if db_key == "olist_db":
    ...
    try:
        ...
        for table_name in inspector.get_table_names():
            ...
            for col in inspector.get_columns(table_name):
                ...
                col_desc = f"Column '{col['name']}' of type {col_type}."
                columns.append({
                    "name": col["name"],
            ...
            schema_tables.append({
                ...
                "description": f"Table '{table_name}' in database '{db_key}'.",
                "columns": columns
            })
        return schema_tables
    except Exception as exc:
        logging.getLogger(__name__).error("Failed to dynamically inspect database %s: %s", db_key, exc)
        return []
