...
def _log_query(
    ...
    try:
        ...
        with get_session("olist_db") as session:
            log_entry = QueryLog(
                question=question,
...
def _execute_sql(sql: str, db_key: str = "olist_db") -> list[dict[str, Any]]:
...
async def run_query(question: str, db_key: str = "olist_db") -> dict[str, Any]:
    """
    ...
    try:
        ...
        schema_context = get_relevant_schema(question, db_key=db_key, k=3)
        ...
        try:
            results = await asyncio.to_thread(_execute_sql, generated_sql, db_key)
        except Exception as exec_error:
            logger.warning("First-pass SQL failed: %s. Attempting self-healing...", exec_error)
            try:
                # Build self-healing prompt
                self_heal_prompt = ChatPromptTemplate.from_messages(
                    [
                        ("system", SYSTEM_PROMPT),
                        ("human", (
                            "Your previous SQL query failed with the following database error:\n"
                            "Error: {error_msg}\n"
                            "Failed Query: {failed_sql}\n\n"
                            "Correct the query based on the schema and output ONLY the corrected SQL SELECT statement."
                        )),
                    ]
                )
                self_heal_chain = self_heal_prompt | llm | StrOutputParser()
                raw_corrected = await self_heal_chain.ainvoke(
                    {
                        "schema": schema_context,
                        "examples": few_shot,
                ...
                corrected_sql = _extract_sql(raw_corrected)
                logger.info("Self-healing generated corrected SQL: %s", corrected_sql)

                # Check corrected SQL safety
                guard_result = check_sql(corrected_sql)
                if guard_result["requires_approval"]:
                    latency_ms = int((time.monotonic() - start_time) * 1000)
                    _log_query(question, corrected_sql, latency_ms, tables_used, error=None)
                    return {
                        "sql": corrected_sql,
                        "results": [],
                        "tables_used": tables_used,
                ...
                results = await asyncio.to_thread(_execute_sql, corrected_sql, db_key)
                generated_sql = corrected_sql
            except Exception as final_error:
                logger.error("Self-healing failed: %s", final_error)
                raise exec_error
