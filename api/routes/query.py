...
async def query_endpoint(request: QueryRequest, x_database: str = Header("olist_db")) -> QueryResponse:
    """
    Convert a natural-language question to SQL and execute it.
    Write operations require explicit approval via /api/approve.
    """
    try:
        result = await run_query(request.question, db_key=x_database)
...
async def approve_endpoint(request: ApproveRequest, x_database: str = Header("olist_db")) -> ApproveResponse:
    """
    ...
    try:
        results = await asyncio.to_thread(_execute_sql, request.sql, db_key=x_database)
